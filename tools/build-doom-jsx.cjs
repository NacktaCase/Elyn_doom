...
//
// tools/build-engine-wasm.cjs 가 `// <<WASM-DATA>>` 를 채우는 것과 **같은
// 마커 패턴**이다. 주입 블록은 생성물이므로 손으로 고치지 말 것.
//
// ── 왜 gzip 인가 ─────────────────────────────────────────────────────
// 정찰(SandboxProbe v1.1)에서 DecompressionStream 이 gzip 을 네이티브로 푸는
// 것을 확인했다. 그래서 원본이 아니라 **압축한 것을 base64 로 싣고 런타임에
