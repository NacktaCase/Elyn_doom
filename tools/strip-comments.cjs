...
//
// **저장소 주석은 그대로 둔다.** 결정 근거가 거기 남아 있고(실기 버그 넷이 전부
// 코어 바깥에서 났다), ChessEngine.jsx 의 `// <<WASM-DATA>>` / `// <<BOOK-DATA>>`
// 마커를 build-engine-wasm.cjs · build-opening-book.cjs · engine-selftest.cjs 가
// 읽는다. 그래서 이 모듈은 dist-elyn/ 로 내보낼 때만 쓴다.
//
